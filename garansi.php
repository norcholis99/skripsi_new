...
<div class="wrapper">
    ...
    <div class="sidebar">
      <nav class="mt-2">
        ...
        <li class="nav-header">BREAK DOWN</li>
          ...
          <li class="nav-item">
            <a href="insentif.php" class="nav-link">
  ...
  <div class="content-wrapper">
    ...
    <section class="content">
     <div class="header">
      ...
      <h4> &nbsp Laporan Kerusakan & Maintenance<br> &nbsp PT. Megah Medika Pharma Cabang Banjarmasin</h4>
    </div>

    
      <div class="button" style="margin: 5px" data-target="#mdl" data-toggle="modal">Tambah Data

    <form method="GET" action="garansi.php" style="float: left;"></div>
  ...
  <div class="modal" tabindex="-1" role="dialog" id="mdl">
      ...
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title">Input Laporan Kerusakan & Maintenance</h5>
      ...
      <form method="POST" action="garansi_tambah.php">
            <div class="container">

          <div class="form-group row mt-3">
            <label class="col-sm-4 col-sm-label">Tanggal</label>
            <div class="col-sm-8"><input type="date" name="no1" class="form-control" required=""></div>
          </div>

          <div class="form-group row mt-3">
            <label class="col-sm-4 col-sm-label">No Berita Acara</label>
          ...
          <div class="form-group row mt-3">
            <label class="col-sm-4 col-sm-label">Masa Pembelian</label>
              <div class="col-sm-8"><select type="text" name="no6" class="form-control" value="option">
                <option value="Dibawah 6 bulan">Dibawah 6 bulan</option>
                <option value="Diatas 6 bulan sampai bulan 12">Diatas 6 bulan sampai bulan 12</option>
                <option value="Diatas 12 bulan">Diatas 12 bulan</option></td></select></div>
              </div>
          ...
          <div class="form-group row mt-3">
            ...
            <div class="col-sm-8"><input type="text" name="no8" class="form-control" required=""></div>
          </div>

          <div class="form-group row mt-3">
            <label class="col-sm-4 col-sm-label">Estimasi Waktu</label>
            <div class="col-sm-8"><input type="text" name="no9" class="form-control" required=""></div>
          </div>

          <div class="form-group row mt-3">
            <label class="col-sm-4 col-sm-label">Estimasi Biaya</label>
            <div class="col-sm-8"><input type="text" name="no10" class="form-control" required=""></div>
          </div>
        </div>
    </div>
  ...
  <div style="margin: 5px">
    ...
    <tr bgcolor="ffff00">
      ...
      <th style="text-align: center;">Masa Pembelian</th>
      <th style="text-align: center;">Keterangan Kerusakan</th>
      <th style="text-align: center;">Jenis Perbaikan</th>
      <th style="text-align: center;">Estimasi Waktu</th>
  ...
  <tbody>
    ...
    <?php 
      ...
      $sql=mysqli_query($kon,"select * from garansi where beritaacara like '%".$cari."%' or namabarang like '%".$cari."%' or pelanggan like '%".$cari."%' order by tanggal asc ");
    } else {$sql = mysqli_query($kon,"select * from garansi order by tanggal asc");
      }
      ...
      <tr>
        ...
        <td style="text-align: center;"><?php echo $saa['beritaacara']; ?></td>
        <td style="text-align: center;"><?php echo $saa['kodebarang']; ?></td>
        <td><?php echo $saa['namabarang']; ?></td>
        <td><?php echo $saa['pelanggan']; ?></td>
        <td><?php echo $saa['masapembelian']; ?></td>
        <td><?php echo $saa['ketrusak']; ?></td>
        <td><?php echo $saa['jenisperbaikan']; ?></td>
        <td style="text-align: center;"><?php echo $saa['eswaktu']; ?></td>
        <td style="text-align: center;"><?php echo $saa['esbiaya']; ?></td>

        <td style="text-align: center;">
              
          <a class="edit.record" href="#" data-target="#md2" data-toggle="modal" data-id=<?php echo $saa ['beritaacara'] ; ?>>
            <i class="nav-icon fas fa-key text-info"></i>
          </a>&nbsp&nbsp
          
          <a href="garansi_hapus.php?id=<?php echo $saa ['beritaacara'];?>">
    ...
    <tr>
      ...
      <div class="modal fade" tabindex="-1" role="dialog" id="md2">
          <div class="modal-dialog" role="document">
            <div class="modal-content">
                <div class="modal-header">
                  <h5 class="modal-title">Edit Data Kerusakan & Maintenance</h5>
                  <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                      <span aria-hidden="true">&times;</span>
                  </button>
                </div>
...
<script>
  $("#tanggal").keyup(function(event){
    if (event.which == 13 || event.keycode == 13){
      $("#beritaacara").focus();
    }
  });
  $("#beritaacara").keyup(function(event){
    if (event.which == 13 || event.keycode == 13){
      $("#kodebarang").focus();
    }
  });
  $("#kodebarang").keyup(function(event){
    if (event.which == 13 || event.keycode == 13){
      $("#namabarang").focus();
    }
  });
  $("#namabarang").keyup(function(event){
    if (event.which == 13 || event.keycode == 13){
      $("#pelanggan").focus();
    }
  });
  $("#pelanggan").keyup(function(event){
    if (event.which == 13 || event.keycode == 13){
      $("#masapembelian").focus();
    }
  });
  $("#masapembelian").keyup(function(event){
    if (event.which == 13 || event.keycode == 13){
      $("#ketrusak").focus();
    }
  });
  $("#ketrusak").keyup(function(event){
    if (event.which == 13 || event.keycode == 13){
      $("#jenisperbaikan").focus();
    }
  });
  $("#jenisperbaikan").keyup(function(event){
    if (event.which == 13 || event.keycode == 13){
      $("#eswaktu").focus();
    }
  });
   $("#eswaktu").keyup(function(event){
    if (event.which == 13 || event.keycode == 13){
      $("#esbiaya").focus();
    }
  });
