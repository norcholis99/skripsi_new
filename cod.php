  ...
  <div class="content-wrapper">
    <section class="content">
      <div class="header">
			...
			<h4> &nbsp Data Tagihan Yang Dibayar Tunai<br> &nbsp PT. Megah Medika Pharma Cabang Banjarmasin</h4>
		</div>
<!-- 
		<div class="button" style="margin: 5px" data-target="#mdl" data-toggle="modal">Tambah Data -->

		<form method="GET" action="#" style="float: left;"><!-- </div> -->
    ...
    <div class="modal" tabindex="-1" role="dialog" id="md5">
      ...
      <form method="GET" action="cod_report.php">
            <div class="container">
	...
	<div class="modal" tabindex="-1" role="dialog" id="mdl">
    	...
    	<div class="modal-content">
      	<div class="modal-header">
        	<h5 class="modal-title">Input Data COD</h5>
        		<button type="button" class="close" data-dismiss="modal" aria-label="Close">
          			<span aria-hidden="true">&times;</span>
        		</button>
      	</div>
      	...
      	<div class="modal-body">
			<form method="POST" action="cod_tambah.php">
      			<div class="container">
	...
	<tbody>
      ...
      // $sql = $kon->query("SELECT * FROM piutang WHERE tgl_jatuhtempo <= '10'");
      // $saa = $sql->fetch_assoc();
      // echo "<script> alert ('hai admin, piutang sudah mau jatuh tempo di ".$saa['no_faktur'].", cepat tagih utangnya') </script>";

			if(isset($_GET['cari'])){ $cari = $_GET['cari'];

			$sql=mysqli_query($kon,"select * from cod where no_faktur like '%".$cari."%' or nama_pelanggan like '%".$cari."%' order by no_faktur desc ");
		} else {$sql = mysqli_query($kon,"select * from cod order by no_faktur desc");
			}
       ...
       <!-- <script>
         SELECT * FROM piutang WHERE (jumlah_tagihan = 0) AND (DATEDIFF(day, tgl_jatuhtempo, GETDATE()) <= 3)
       </script> -->


			<tr>
        <td style="text-align: center;"><?php echo $saa['no_faktur']; ?></td>
				...
				<td style="text-align: right;"><?php echo "Rp " . (number_format($saa['jumlah_tagihan'],0,',','.')); ?></td>
            ...
            <i class="nav-icon fas fa-key text-info"></i>
					...
					<a href="pt_hapus.php?id=<?php echo $saa ['no_faktur'];?>">
		...
		<tr>
			...
			<div class="modal fade" tabindex="-1" role="dialog" id="md2">
  				<div class="modal-dialog" role="document">
    				<div class="modal-content">
      					<div class="modal-header">
        					<h5 class="modal-title">Edit Data COD</h5>
        					<button type="button" class="close" data-dismiss="modal" aria-label="Close">
          						<span aria-hidden="true">&times;</span>
        					</button>
      					</div>
    ...
    $query=mysqli_query($kon,"select sum(jumlah_tagihan) as jumlah_tagihan from cod");
    ...
    <th colspan="5" style="text-align: center; font-size: 12px">Total</th>
    <th style="text-align: right; font-size: 12px"><?php echo "Rp".(number_format($total,0,',','.')); ?></th>
