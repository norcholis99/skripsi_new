...
$pdf->SetFont('Arial','',10);
$pdf->SetX(4);
$pdf->MultiCell(95,10,'Laporan Data Movement On Periode Bulan '.$bln_list[date('m')].' '.date('Y'),0,'C');

$pdf->SetFont('Arial','B',10);
$pdf->Cell(8,6,'No',1,0,'C');
$pdf->Cell(18,6,'No Mo',1,0,'C');
...
if ($_GET['tanggal1']) {
    ...
    $tgl=$_GET['tanggal1'];
    $tgl2=$_GET['tanggal2'];
$movement_on_2 = mysqli_query($kon, "select * from movement_on where tanggal ber=tween '$tgl' and '$tgl2' ");
while ($row = mysqli_fetch_array($movement_on_2)){
    $pdf->Cell(8,6,$row['no'],1,0,'C');
    $pdf->Cell(18,6,$row['no_mo'],1,0);
...
$movement_on = mysqli_query($kon, "select * from movement_on order by no asc");
